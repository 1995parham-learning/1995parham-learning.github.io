...
To set these `status.conditions` for the Pod, **applications** and **operators** should use the _PATCH_ action. You can use a _Kubernetes client library_ to write code that sets custom Pod conditions for Pod readiness.
...
![[Pasted image 20241130222428.png]]
...
[`kube-proxy`](https://kubernetes.io/docs/reference/command-line-tools-reference/kube-proxy/) maintains network rules on nodes. These network rules allow network communication to your Pods from network sessions inside or outside your cluster.
